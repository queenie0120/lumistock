...
▸ cat（新聞 6 類分類，規格四）
- 個股：財報、法說、併購、新品、訂單、裁員、高層異動、升降評、訴訟
- 產業：AI / 半導體 / 雲端 / 電動車 / 軟體 / 金融 / 消費 / 能源 / 醫療 / 國防
- 總經：Fed 利率 / CPI / PPI / 非農 / 失業率 / 美債殖利率 / 美元 / 通膨 / GDP
- 市場：S&P / Nasdaq / Dow / Russell / SOX / VIX / ETF 資金流
- 國際：地緣 / 戰爭 / 關稅 / 制裁 / 原油 / 黃金 / 匯率 / 斷鏈
- 台連：美科技 → 台電子；費半 → 台半導體；TSM ADR / NVDA 等對台股供應鏈
（必選一個，若混合取主要那個）
...
[
  {"i":1,"imp":"high/mid/low","sent":"偏多/偏空/中性/觀望/短多長空/短空長多",
   "impact":"...","priced":true/false,"risk":"...","cat":"個股/產業/總經/市場/國際/台連",
   "why":"重要度 1 句原因"},
  ...
]
...
def ai_enrich_news_items(items: list, timeout: int = 20) -> list:
    """v10.9.169 → v10.9.175 改名：擴增批次 AI 判讀（給「分類大盤新聞」用）
    回傳 items（每筆加上 importance / sentiment / impact / already_priced / risk / category / importance_reason）
    AI 失敗時保持原列表，不影響顯示。

    ⚠ v10.9.175 修：原本叫 ai_analyze_news_batch 撞既有同名函數（line 8640，
    給「個股新聞 AI 過濾」用，回 keep/sentiment/summary 結構），導致 v10.9.169
    起 get_tw_stock_news_with_ai / get_us_stock_news_with_ai 等呼叫 TypeError。
    本函數改名後，原 ai_analyze_news_batch unshadow 恢復正常。"""
...
# v10.9.166 → v10.9.169 → v10.9.175：別名（向後相容；舊呼叫端不會壞）
def ai_classify_news_importance(items: list, timeout: int = 15) -> list:
    return ai_enrich_news_items(items, timeout=timeout)
...
def make_news_carousel(title: str, color: str, items: list,
    ...
    for i, n in enumerate(items[:10], start=1):
        ...
        # v10.9.173：分類 badge
        if cat and cat in cat_emoji:
            if badge_row:
                badge_row.append({"type": "text", "text": "  ",
                                  "size": "xxs", "color": "#FDF6F0", "flex": 0})
            badge_row += [
                {"type": "text", "text": f"{cat_emoji[cat]} {cat}",
                 "size": "xxs", "color": "#A05A48", "weight": "bold", "flex": 0},
            ]
        if badge_row:
            body_contents.append({
